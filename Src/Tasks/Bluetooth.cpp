...
void Bluetooth_Process(void) {
	...
	if (signal_type.value.signals == 0x01) {
		...
		case 'q':
			...
			terminal_length = sprintf((char *) terminal_tx, "w:%d\tx:%d\ty:%d\tz:%d\n", (int16_t)(orientation.w * 32767.f), (int16_t)(orientation.x * 32767.f), (int16_t)(orientation.y * 32767.f), (int16_t)(orientation.z * 32767.f));
			HAL_UART_Transmit_DMA(&huart8, terminal_tx, terminal_length);
			break;
		case 'o':
			terminal_length = sprintf((char *) terminal_tx, "Odometry x: %f\tOdometry y: %f\n", odometry.getX(), odometry.getY());
			HAL_UART_Transmit_DMA(&huart8, terminal_tx, terminal_length);
			break;
		}
		HAL_UART_Receive_IT(&huart8, terminal_rx, 1);
